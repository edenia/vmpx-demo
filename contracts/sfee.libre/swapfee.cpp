...
int swapfee::median(symbol_code pair_token)
{
    ...
    if (sum == 0)
        return FEE_VECTOR.at(DEFAULT_FEE_INDEX);
...
void swapfee::updatefee(symbol_code pair_token)
{
    int new_fee = median(pair_token);
    action(permission_level{get_self(), "active"_n},
           "swap.libre"_n, "changefee"_n,
           make_tuple(pair_token, new_fee))
        .send();
}

void swapfee::onaddliquidity(name user, asset to_buy, asset max_asset1, asset max_asset2)
{
    add_balance(user, to_buy, true);
}

void swapfee::onremliquidity(name user, asset to_sell, asset min_asset1, asset min_asset2)
{
    add_balance(user, -to_sell, true);
}

void swapfee::ontransfer(const name &from, const name &to, const asset &quantity, const string &memo)
{
...
{
    accounts table("swap.libre"_n, user.value);
    const auto &user_balance = table.find(pair_token.raw());
    check(user_balance != table.end(), "pair_token balance does not exist");
    return user_balance->balance;
}

void swapfee::votefee(name user, symbol_code pair_token, int fee_voted)
{
    check((FEE_VECTOR.at(MIN_FEE_INDEX) <= fee_voted) &&
              (fee_voted <= FEE_VECTOR.at(MAX_FEE_INDEX)),
          "only values between 10 and 100 are allowed");
...
void swapfee::closevote(name user, symbol_code pair_token)
{
    require_auth(user);
    feeaccounts acnts(get_self(), user.value);
    auto acnt = acnts.find(pair_token.raw());
...
void swapfee::closefeetable(symbol_code pair_token)
{
    feetables tables(get_self(), pair_token.raw());
    auto table = tables.find(pair_token.raw());
    ...
    auto is_empty = all_of(votes.begin(), votes.end(),
                           [](const int &votes_number)
                           { return votes_number == 0; });
    check(is_empty, "table of votes is not empty");
    tables.erase(table);
}

void swapfee::openfeetable(name user, symbol_code pair_token)
{
    feetables tables(get_self(), pair_token.raw());
    auto table = tables.find(pair_token.raw());
...
void swapfee::add_balance(name user, asset to_add, bool need_update)
{
    feeaccounts acnts(get_self(), user.value);
...
void swapfee::addvote(symbol_code pair_token, int fee_index, int64_t amount,
                      bool need_update)
{
    feetables tables(get_self(), pair_token.raw());
    auto table = tables.find(pair_token.raw());
...
int swapfee::get_index(int fee_value)
{
