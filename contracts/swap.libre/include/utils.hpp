...
string_view trim( string_view sv ) {
  sv.remove_prefix(
      std::min( sv.find_first_not_of( " " ), sv.size() ) ); // left trim
  sv.remove_suffix( std::min( sv.size() - sv.find_last_not_of( " " ) - 1,
                              sv.size() ) ); // right trim
  return sv;
}
...
template < class T > void to_int( string_view sv, T &res ) {
  res = 0;
  T p = 1;
...
template < class T > void precision_from_decimals( int8_t decimals, T &p10 ) {
...
asset asset_from_string( string_view from ) {
  ...
  if ( dot_pos != string::npos ) {
    check( dot_pos != amount_str.size() - 1,
